   ...
   spentFilter(filter) {
      this.filteredList = this.cardList.filter((ele) => {
         return ((filter.searchText != '' ? ele.name
          ...
          (ele.spent.value <= filter.spent) && (filter.burner ? ele.card_type === 'burner' : filter.subscription ? ele.card_type === 'subscription' : true));
      })
    }
  },
  async created() {
    ...
    data = data.filter((ele) => {
      //assuming that my user id is 1
      return ele.owner_id === 1;
    });
